...
public class pZinp : MonoBehaviour
{
    ...
    public void onInputChange(string str)
    {
        if (DetEvents.canModify)
        {
            ...
            try
            {
                ...
                Vector3 newVal = DetEvents.selected.props.position;
                newVal.z = parsed;
                DetEvents.selected.modifyPosition(newVal);
            }
            catch (Exception)
            {
                input.text = getText();
    ...
    {
        return DetEvents.selected.pos.z.ToString();
    }
}
